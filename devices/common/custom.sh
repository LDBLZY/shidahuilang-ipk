...
status=$(curl -H "Authorization: token $REPO_TOKEN" -s "https://api.github.com/repos/shidahuilang/packages/actions/runs" | jq -r '.workflow_runs[0].status')
...
./scripts/feeds install -a -p shidahuilang -f
./scripts/feeds install -a

rm -rf feeds/packages/lang/golang
svn export https://github.com/coolsnowwolf/packages/trunk/lang/golang feeds/packages/lang/golang

sed -i 's/\(page\|e\)\?.acl_depends.*\?}//' `find package/feeds/shidahuilang/luci-*/luasrc/controller/* -name "*.lua"`
sed -i 's/\/cgi-bin\/\(luci\|cgi-\)/\/\1/g' `find package/feeds/shidahuilang/luci-*/ -name "*.lua" -or -name "*.htm*" -or -name "*.js"` &
...
sed -i \
	...
	package/feeds/shidahuilang/*/Makefile
